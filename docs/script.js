// ===================================================
// Nebula Chat — script.js (GitHub Pages version)
// Backend lives on Railway. Replace RAILWAY_URL below
...
async function loadChatHistory() {
    ...
    try {
        const res  = await fetch(`${RAILWAY_URL}/api/messages/${encodeURIComponent(currentUser)}/${encodeURIComponent(selectedUser)}`);
        const data = await res.json();
...
async function addMemberToGroup(groupId, username) {
    try {
        const res = await fetch(`${RAILWAY_URL}/api/groups/${groupId}/join?username=${encodeURIComponent(username)}`, {
            method: 'POST'
        });
        if (res.ok) {
